...
Desktop Mode 免登录已在 Phase 1 MVP 实现：
...
开发包阶段使用 Go 编译的 `LazyMind.exe` 替代 `LazyMind.bat`：
...
- 启动流程与原 bat 文件等价：设置环境变量 → 启动 `core.exe`（隐藏窗口）→ 轮询健康检查 → 启动 `electron.exe` → 等待 Electron 退出 → 清理 `core.exe` 进程。
- 子进程使用 `CREATE_NO_WINDOW` 标志启动，避免闪现控制台。
- Electron 退出后通过 `taskkill /T /F /PID` 清理 core.exe 进程树。

安装包阶段由 electron-builder 产出最终带签名的 `LazyMind.exe`（Electron 本体），此 Go launcher 仅用于开发包和无安装器分发场景。
