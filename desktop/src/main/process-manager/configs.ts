...
export function getServiceConfigs(
  ...
  return [
    ...
    },
    {
      ...
      executablePath: `${binDir}/python`,
      args: ['-m', 'uvicorn', 'backend.algorithm.main:app', '--host', '127.0.0.1', '--port', String(SERVICE_PORTS.algorithmService)],
      env: {
        LAZYMIND_LOCAL_SECRET: localSecret,
        SERVER_HOST: '127.0.0.1',
        SERVER_PORT: String(SERVICE_PORTS.algorithmService),
        ALGO_DB_PATH: `${sqliteBase}/algo.db`,
        SEGMENT_DB_PATH: `${sqliteBase}/segment.db`,
        VECTOR_DIR: `${dataDirRoot}/vector`,
        CORE_URL: `http://127.0.0.1:${SERVICE_PORTS.core}`,
        CREDENTIAL_BRIDGE_URL: `http://127.0.0.1:${SERVICE_PORTS.proxy}`,
      },
      healthCheck: {
        type: 'http',
        endpoint: `http://127.0.0.1:${SERVICE_PORTS.algorithmService}/health`,
        intervalMs: PROCESS_CONFIG.healthCheckIntervalMs,
        timeoutMs: PROCESS_CONFIG.healthCheckTimeoutMs,
        retries: 3,
      },
      port: SERVICE_PORTS.algorithmService,
      dependsOn: ['core'],
      startupTimeout: PROCESS_CONFIG.startupTimeoutMs,
      restartPolicy: 'on-failure',
      maxRestarts: PROCESS_CONFIG.maxRestarts,
